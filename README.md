...
이번 준비 단계에서는 실제 legacy 기능을 구현하지 않습니다. 로그인, 회원가입, 장비 목록, 지도, 파일 업로드, DB 마이그레이션, 인증은 다음 단계에서 설계 후 추가합니다.
...
```powershell
cd C:\workspace\oursourcing\jonard-cloud\frontend
npm install
npm run dev
```
...
```powershell
cd C:\workspace\oursourcing\jonard-cloud\backend
py -m venv .venv
